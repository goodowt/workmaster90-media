#!/usr/bin/env python3
"""카드 PNG(1.png..N.png) 를 켄번스 + 크로스페이드 영상(4:5, 1080x1350)으로 만든다.
...
def zoom_expr(k: int):
    ...
    if k % 3 == 1:  # zoom out
        return f"1+{ZMAX}-{ZMAX}*on/{DUR}", "iw/2-(iw/zoom/2)", "ih-(ih/zoom)"
    return f"1+{ZMAX}", f"(iw-iw/zoom)*on/{DUR}", "ih-(ih/zoom)"  # pan
